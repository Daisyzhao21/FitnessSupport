...
col_c.metric("📊 净摄入", f"{total_calories - total_burned:.0f} kcal")
    ...
    if mode == "🔍 手动":
        ...
        if term:
            ...
            for _, row in results.iterrows():
                ...
                qty = cols[2].number_input(config['label'], config['min'], config['max'], config['default'], config['step'], key=f"qty_{row['名称']}", label_visibility="collapsed")
